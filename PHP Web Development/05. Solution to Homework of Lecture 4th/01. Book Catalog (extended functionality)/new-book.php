...
?>
    ...
    <div style="float: left; padding-top: 2px">
        <a href="index.php">Book Catalog</a> | <b>New Book</b> | <a href="new-author.php">New Author</a>
    </div>
...
if (isset($_POST['new-book']))
{
    ...
    if (!$error)
    {
        $book_title = htmlspecialchars(mysqli_real_escape_string($CONNECTION_BOOKS, $book_title));

        // Извличане на името на книгата
        $query = mysqli_query($CONNECTION_BOOKS, 'SELECT book_title FROM books WHERE book_title = "' . $book_title . '"');
        if (HasErrorWithDataBase($query)) exit; // Проверка за грешки
        ...
        else // Добавяме книгата и я свързваме с нейните автори
        {
            ...
            $book_id = mysqli_insert_id($CONNECTION_BOOKS);

/* Деклариране на структура (извън Main) Position с полета row и col + конструктор;
 
*Създаване на инстанция rndNumGen(извън Main) от тип Random;

*Задаване размери на конзолния буфер;

*Създаване на инстанция food от тип Position, като полета задаване случайни числа в интервал текущия конзолен прозорец;
 
Деклариране на опашка Queue, която съдържа елементи на структурата Position;

Деклариране масив directions от тип Position и в него създаваме 4 инстанции на структурата Position(row,col);
 
Деклариране променлива direction и текущата позиция (накъде ще се движи змията);

Добавяме в цикъл 5 елемента - инстанции на структурата Position(0,i);

---БЕЗКРАЕН ЦИКЪЛ---

Проверяваме дали е натиснат клавиш;
-> четем и пазим този клавиш в променлива от тип ConsoleKeyInfo;
-> проверяваме кой е натиснатия клавиш:
- надясно -> direction = 0;
- наляво -> direction = 1;
- надолу -> direction = 2;
- нагоре -> direction = 3;

В променлива от тип Position отделяме последния елемент на опашката (главата);

В променлива изчисляваме новата позиция на главата (използваме масива directions и достъпваме елементите от него по стойността на direction;

Създаваме инстанция, която ще съдържа новите координати на глава, като row = 6.row + 7.row, col = 6.col + 7.col;

Проверяваме дали координатите на новата глава са стигнали границата на масива (конзолата)
-> Задаваме начални координати на курсора
-> Изписваме съобщение
-> Прекратяване на главната фукция
    
Добавяне на новата глава към опашката чрез фукцията Enqueue;
 
Проверяваме дали координатите на главата на змията съвпадат с координатите на food;
-> Ако съвпадат, генерираме нови координати на food
-> В противен случай - премахни първия елемент на опашката
... когато изяде храната не се премахва елемент, при всички други случаи (при движение) се премахва;
 
Изчистване на конзолата;
 
Изобразяване на food:
-> Задаване координатите на food като координати на курсора
-> Изобразяване на символ

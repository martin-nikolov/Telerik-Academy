...
// However, SMs generators use the Python-inspired semantics of
// outdated ES6 drafts.  We would like to support ES6, but wed also
...
function defer() {
    ...
    if (Q.longStackSupport && hasStacks) {
        try {
            ...
            // NOTE: dont try to use `Error.captureStackTrace` or transfer the
            // accessor around; that causes memory leaks as per GH-111. Just
            // reify the stack trace as a string ASAP.
            //
            // At the same time, cut off the first line; its always just
            // "[object Promise]\n", as per the `toString`.
    ...
    // consolidating them into `become`, since otherwise wed create new
...
Q.getUnhandledReasons = function () {
    // Make a copy so that consumers cant interfere with our internal state.
 ...
 * handler. This is useful because its extremely common to run
 ...
 * promises originating deferred. This listener receives the exact arguments
